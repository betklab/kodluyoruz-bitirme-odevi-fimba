...
public class WarehouseCRUDServiceImpl implements WarehouseCRUDService {
    ...
    public WarehouseAPIResponseHolder<?> transfer(Long fromId, Long toId) {
        boolean isActive = warehouseOperationRepository.isActiveWarehouses(fromId, toId);

        if (!isActive) {
            return new WarehouseAPIResponseHolder<>(HttpStatus.NO_CONTENT, WarehouseAPIResponseError
                    .builder()
                    .code("NOT_ACTIVE_WAREHOUSES")
                    .message("You can not transfer from/to inactive warehouses")
                    .build()
            );
        }
        warehouseOperationRepository.transfer(fromId, toId);
        return new WarehouseAPIResponseHolder<>(HttpStatus.OK);
    }
}
